...
logging.basicConfig(level=logging.INFO, filename='dataproc1_evaluation.log', filemode='w')
...
def check_bigquery_table(student_id):
    ...
    dataset = 'logs_shared'
    table = f'analytics_results.error_summary_{student_id}'
...
def check_bigquery_schema(table):
    global PASS, FAIL
    expected = {'app_name', 'error_type', 'error_count'}
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python evaluate_dataproc1.py <student_id>")
        sys.exit(1)
