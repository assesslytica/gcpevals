...
logging.basicConfig(level=logging.INFO, filename='etl_evaluation.log', filemode='w')

STUDENT_ID = os.environ.get('STUDENT_ID')
BQ_DATASET = 'retail_shared'
BQ_TABLE = 'sales_raw'
SPANNER_INSTANCE = 'retail-shared-instance'
SPANNER_DATABASE = 'retail_shared_db'
SPANNER_TABLE = f'sales_summary_{STUDENT_ID}'
...
def check_spanner_schema():
    ...
    with database.snapshot() as snapshot:
        ...
        expected = {('store_id', 'STRING'), ('sale_date', 'DATE'), ('total_sales', 'FLOAT64')}
...
def check_dataflow_job():
    # Placeholder: Instruct students to provide Dataflow job ID as env var or output file
    job_id = os.environ.get('DATAFLOW_JOB_ID')
    if not job_id:
        logging.warning("DATAFLOW_JOB_ID not provided. Skipping Dataflow job check.")
        return False
    # Optionally, use Dataflow API to check job status
