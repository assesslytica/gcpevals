...
def main():
    ...
    # Task 3: Check at least 5 valid messages published
    try:
        ...
        for msg in response.received_messages:
            try:
                ...
                if all(field in data for field in ["transaction_id", "user_id", "amount", "currency", "timestamp"]):
                    valid_count += 1
            except Exception:
                continue
        if valid_count >= 5:
            log(f"PASS: At least 5 valid messages found in subscription.")
            PASS += 1
        else:
            log(f"FAIL: Less than 5 valid messages found in subscription.")
            FAIL += 1
    except Exception as e:
